...
export default function ProductCompositions() {
  ...
  const loadCompositions = useCallback(async () => {
    ...
    try {
      ...
      if (data.length === 0) {
        showToast(
          "info",
          "Nenhum ingrediente encontrado. Adicione o primeiro!",
        );
      }
    } catch (error) {
  ...
  function handleEditComposition(item: any) {
    router.push({
      pathname: "/(private)/stock/product-composition/edit",
      params: {
        id: item.id,
        name: item.name,
        unit: item.unit,
        ...
        <View style={{ flex: 1 }}>
          ...
          <S.ItemInfo>
            ...
            <Text style={{ fontWeight: "600" }}>
              {item.kitchen?.name ?? "Não definida"}
            </Text>
          </S.ItemInfo>
        </View>
              ...
              <FlatList
                ...
                ListEmptyComponent={
                  <S.EmptyContainer>
                    ...
                    <S.EmptyText>Nenhum ingrediente adicionado</S.EmptyText>
                    <S.EmptySubtext>
